<div class="w-full">
    ...
    <!-- Modal Content -->
    <h3 class="text-xl font-semibold text-gray-800">Bidding Request</h3>
    <!-- Toastr CSS -->
                                ...
                                <td class="px-6 py-0">
                                    ...
                                    @php
                                        $ds = DB::table('loading_rattings')
                                            ...
                                            ->first();
                                        $insurance = DB::table('insurances')->where('load_id', $result->id)->first();
                                    @endphp

                                    @if ($result->status == 'Delivered')
                                        <div class="flex gap-3">
                                            @if ($result->payment_status != 'completed')
                                                <button wire:click="PaymentModel({{ $result->id }})"
                                            ...
                                            @if ($result->payment_status == 'completed')
                                                @if (!$insurance)
                                                    <button wire:click="InsuranceModel({{ $result->id }})"
                                                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-1 rounded-md">
                                                        Insurance
                                                    </button>
                                                @endif
                                            ...
                                            @if (!$ds)
                                                <button wire:click="bookingModel({{ $result->id }})"
                                                    class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-1 rounded-md">
                                                    Rating
                                                </button>
                                            @endif
                                    ...
                                    @if ($result->status == 'pending')
                                        <button wire:click="booking({{ $result }})"
                                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-1 rounded-md">
                                            Status Change
                                        </button>
                                    @endif
    ...
    <div x-show="$wire.modelRatting" class="fixed inset-0 flex items-end justify-end bg-gray-800 bg-opacity-50"
        style="display: none;">
        <div style="width: 300px;" class="text-sm h-full max-w-xs md:max-w-none bg-white p-6 rounded-r-lg shadow-lg">
            <!-- Modal Content -->
            <h3 class="text-xl font-semibold">Bid Review</h3>
            <form wire:submit.prevent="bookingApproved" class="w-full mt-10">
            ...
            <button wire:click="closeModelRatting"
                class="mt-4 px-6 py-2 text-white bg-red-600 rounded-lg w-full">Close</button>
        </div>
    </div>

    <div x-show="$wire.modelInsurance" class="fixed inset-0 flex items-end justify-end bg-gray-800 bg-opacity-50"
        style="display: none;">
        <div style="width: 300px;" class="text-sm h-full max-w-xs md:max-w-none bg-white p-6 rounded-r-lg shadow-lg">
            <!-- Modal Content -->
            <h3 class="text-xl font-semibold">Insurance Claim</h3>
            <form wire:submit.prevent="bookingApprovedImage" class="w-full mt-10">
                <label for="equipment" class="block text-gray-700 font-medium mt-2">Comment</label>
                <input type="text" wire:model="commentInsurance" placeholder="comment"
                    class="mt-1 p-2 w-full border rounded">
                ...
                <input type="file" wire:model="images" id="images" class="mt-1 p-2 w-full border rounded"
                ...
                <!-- Display preview of selected images (optional) -->
                @if ($images)
                    <div class="mt-4">
                        @foreach ($images as $image)
                            <img src="{{ $image->temporaryUrl() }}" class="w-20 h-20 object-cover mr-2 mb-2" />
                        @endforeach
                    </div>
                @endif
            ...
            <button wire:click="closeModelInsurance"
                class="mt-4 px-6 py-2 text-white bg-red-600 rounded-lg w-full">Close</button>
        </div>
    </div>
